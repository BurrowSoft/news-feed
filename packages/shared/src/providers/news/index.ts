...
const SEA_COUNTRIES = new Set(["TH", "MY", "SG", "ID", "PH", "VN", "KH", "LA", "MM"]);
const LATAM_COUNTRIES = new Set(["BR", "AR", "CL", "CO", "MX", "PE", "UY", "VE"]);
...
export function createNewsRouter(
  ...
  if (isTH) {
    ...
    // Guardian: no Thai content — skipped
  ...
  if (isBR) {
    ...
    if (process.env.NEWS_API_KEY)
      providers.push(new NewsAPIProvider(process.env.NEWS_API_KEY));
    // Guardian: no Portuguese content — skipped
    return new ProviderRouter(providers);
  }

  // --- US + all others (English default) ---
  if (process.env.NEWS_API_KEY)
    providers.push(new NewsAPIProvider(process.env.NEWS_API_KEY));
  if (process.env.GNEWS_API_KEY)
    providers.push(new GNewsProvider(process.env.GNEWS_API_KEY));
  ...
  // For non-English countries with key support, add NewsData/MediaStack
